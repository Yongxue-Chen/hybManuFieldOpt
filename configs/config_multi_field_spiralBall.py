...
WEIGHTS = {
    'final_state': 2.0,
    'allowed_final_state': 0.1,

    'self_support': 1.093,
    'allowed_self_support': 0.002,

    'AM_Collision_Free': 0.007306,
    'allowed_AM_Collision_Free': 0.001,

    'SM_Collision_Free': 0.04639,
    'SM_Below_AABB': 100.0,

    'operation_volume': 0.01912,
    'allowed_operation_volume': 0.4,
    
    'structure': 0.0002862,
    'allowed_structure': 100.0,
    'structure_simp_penalty': 0.0001,
}

BAYESIAN_WEIGHT_SEARCH_SPACE = {
    # 'final_state':              (1.0, 100.0, False),
    'self_support':             (0.1, 10.0, True),
    'AM_Collision_Free':        (0.001, 1.0, True),
    'SM_Collision_Free':        (0.01, 5.0, True),
    'operation_volume':         (0.01, 5.0, True),
    'structure':                (0.0001, 1.0, True),
}
...
BAYESIAN_METRIC_TARGETS = {
    'final_state_accuracy':      (0.985, 'higher'),   # 98% of empty points correctly classified
    ...
    'SM_collision_free_ratio':   (0.80, 'higher'),   # 95% of SM points collision-free
    'operation_volume_ratio':    (0.80, 'higher'),   # 90% of empty points not over-added
    'structure_normalized':      (1e-3, 'lower'),  # per-grid structure penalty threshold
}
