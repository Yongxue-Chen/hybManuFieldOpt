...
class StructureLossCalculatorTorch:
    """
    纯 PyTorch 实现的结构拓扑优化 / 结构安全损失计算器。

    核心思想：
        - 使用有限差分近似的线弹性模型 + 共轭梯度法 (CG) 求解在重力载荷下的位移场 u；
        - 对超过允许下沉量的体素施加二次惩罚，构造“位移越界”型损失；
    ...
    def __init__(self, 
        ...
        print(f"[StructureLossCalculatorTorch] Initialized with:")
    ...
    def _compute_loss(self, u, rho):
        ...
        #   2) 下沉超过允许阈值 (drop_distance > max_displacement)
        with torch.no_grad():
            mask_material = (rho > self.rho_threshold).float()
            mask_fail = (drop_distance > self.max_displacement).float()
        ...
        excess_drop = torch.relu(drop_distance - self.max_displacement)
        penalty_field = rho * (excess_drop ** 2)
    ...
    def structureLoss(self, samplePoints, density):
        """
        结构拓扑优化 / 结构安全损失函数入口。
        ...
        grid_4d = grid_4d.to(self.device)
...
class StructureFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, density, calculator):
        """
        ...
        Args:
            ...
            calculator: StructureLossCalculatorTorch
    ...
    def backward(ctx, grad_loss):
        ...
        # === 1. 重算中间变量 (为了得到 Mask 和 excess_drop) ===
        u_z = u[..., 2]
        drop_distance = -u_z
        
        with torch.no_grad():
            mask_material = (density > calculator.rho_threshold).float()
            mask_fail = (drop_distance > calculator.max_displacement).float()
            mask_target = mask_material * mask_fail
            
        excess_drop = torch.relu(drop_distance - calculator.max_displacement)
        ...
        # 公式: (1/M) * mask * rho * 2 * excess * (-1)
        # 这是一个把物体"往上拉"的虚拟力
        dL_du_z = mask_target * density * 2 * excess_drop * (-1.0)
        # 归一化：除以总质量
        dL_du_z = dL_du_z / total_mass
        dL_du_z = dL_du_z * batch_mask_view

        # 组装成矢量 [B, X, Y, Z, 3]
        dL_du = torch.zeros_like(u)
        dL_du[..., 2] = dL_du_z
        ...
        with torch.no_grad():
            grad_density = calculator._compute_adjoint_gradient(
                u, lambda_vec, density, excess_drop, mask_target, total_mass, batch_mask_view
            )
